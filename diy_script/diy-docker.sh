...
sed -i \
    ...
    ./feeds/packages/utils/docker/Makefile
...
# ============================================================
# Docker CLI 29.8.0 jsonschema/v6 修复
# ============================================================
#
# Docker CLI 29.8.0 的：
#
...
if [ -f "$DOCKER_MAKEFILE" ]; then
    ...
    # Remove previous version of this fix if it exists.
    sed -i \
        '/# OPENWRT_DOCKER_JSONSCHEMA_FIX_V6/,/^endef$/d' \
        "$DOCKER_MAKEFILE" 2>/dev/null || true

    # --------------------------------------------------------
    # Add jsonschema fix function
    # --------------------------------------------------------
